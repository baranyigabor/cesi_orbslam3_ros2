...
{   
    ...
    private:
        ...
        std::string subTerminationMsgName = "";
        std::string poseMsgName = "";
        std::string syncMsgName = "";
        std::string depthMsgName = "";
        ...
        rclcpp::Subscription<std_msgs::msg::String>::SharedPtr subTerminationMsg_subscription_;
        rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr subImgMsg_subscription_;
        rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr subDepthMsg_subscription_;
        ...
        bool newRGBImg = false;
        bool newDepthImg = false;
        int time_step = 0;
        //* ROS callbacks
        ...
        void processImages();
        void termination_callback(const std_msgs::msg::String& msg);
        //* Helper functions
        ...
        void savePose(const Sophus::SE3f& Tcw, double timestamp, int frameNumber);

        struct TimingStats {
            std::vector<double> frameTimes;
            std::vector<bool> trackingSuccesses;
            std::vector<int> numTrackedPoints;
            double totalTime;
            ...
            double avgFrameProcessingTime() const {
                if (frameTimes.empty()) return 0.0;
                return std::accumulate(frameTimes.begin(), frameTimes.end(), 0.0) / 
                    frameTimes.size();
            }
            
            double avgTrackedPoints() const {
                if (numTrackedPoints.empty()) return 0.0;
                return std::accumulate(numTrackedPoints.begin(), numTrackedPoints.end(), 0.0) / 
                    numTrackedPoints.size();
            }
        };
        ...
        int endFrame;
        // Timing stats
        TimingStats mStatsMonocular;
        TimingStats mStatsRGBD;
        std::chrono::time_point<std::chrono::high_resolution_clock> mStartTime;
        bool mSaveTiming;
        std::string mStatsFileName;
        
        // Functions to save timing stats
        void saveTimingStats();
        
        // Test methods
        void runTests();
        void runMonocularTest();
        void runRGBDTest();
        double calculateTrajectoryError(const std::vector<Sophus::SE3f>& trajectory, 
                                    const std::string& gtFile);
        void saveTestResults();
        void saveTrajectory(const std::string& mode, const std::vector<Sophus::SE3f>& poses);
};
