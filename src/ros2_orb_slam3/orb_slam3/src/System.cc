    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    //Check settings file
    cout << strSettingsFile.c_str() << "alma" << endl;
...
TrackingOutput System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    TrackingOutput track_out;
    if (mpTracker->detectionState != nullptr) {
        //std::cout << "The value of detectionState is: " << *(mpTracker->detectionState) << std::endl;
        track_out.isTrackingSuccessful = *(mpTracker->detectionState);
    } else {
        //std::cout << "detectionState is null" << std::endl;
        track_out.isTrackingSuccessful = false;
    }
    //cout << "detection state: " << *(mpTracker->detectionState) << endl;

    track_out.Tcw = mpTracker->GrabImageMonocular(imToFeed,timestamp,filename);
    track_out.mvpMapPoints = mpTracker->mCurrentFrame.mvpMapPoints;
    return track_out;
}
