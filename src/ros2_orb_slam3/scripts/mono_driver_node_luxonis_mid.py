...
class OAKDPublisher(Node):
    def __init__(self, node_name = "mono_py_node"):
        ...
        intrinsics = calibData.getCameraIntrinsics(dai.CameraBoardSocket.CAM_A, 1280, 800)
    ...
    def publish_frames(self):
        while True:
            ...
            # Convert and publish RGB image
            rgb_msg = self.bridge.cv2_to_imgmsg(cv2.resize(temp_rgb.getCvFrame(), (1280, 800)), encoding="bgr8")
            rgb_msg.header.stamp = self.get_clock().now().to_msg()
            self.rgb_pub.publish(rgb_msg)
