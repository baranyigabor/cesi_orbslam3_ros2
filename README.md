...
## NIPG related changes ---------------------------------------------------------

## Set environment configs -- ROS2
- ```export RMW_IMPLEMENTATION=rmw_fastrtps_cpp```
- ```export ROS_DOMAIN_ID=30```
- ```source /opt/ros/humble/setup.bash```

## Run ORB-SLAM3 module
- ```ros2 run ros2_orb_slam3 mono_node_cpp --ros-args -p node_name_arg:=mono_slam_cpp```

## Run Camera stream

- ```ros2 run ros2_orb_slam3 mono_driver_node.py --ros-args -p settings_name:=Bosch_vid1 -p image_seq:=sample_euroc_MH05```

example Orb-SLAM3config files are available at: ```src/ros2_orb_slam3/orb_slam3/config/Monocular/```